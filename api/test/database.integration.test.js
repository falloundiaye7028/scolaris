...
test("connexion, limitation, sessions, RBAC et isolation multi-établissements", { skip: !databaseUrl }, async (context) => {
  ...
  const { assertPreviewDatabaseIdentity } = await import("../scripts/preview-seed-guard.mjs");
  ...
  const previewSeedEnvironment = { VERCEL: "1", VERCEL_ENV: "preview", SCOLARIS_ALLOW_PREVIEW_SEED: "1", SCOLARIS_PREVIEW_DATABASE_FINGERPRINT: "a".repeat(64) };
  await admin.query("INSERT INTO deployment_environment_identity(singleton,environment,resource_fingerprint) VALUES(true,'production',$1)", ["a".repeat(64)]);
  await assert.rejects(assertPreviewDatabaseIdentity(admin, previewSeedEnvironment), /IDENTIFIED_AS_PRODUCTION/);
  await admin.query("UPDATE deployment_environment_identity SET environment='preview',resource_fingerprint=$1", ["b".repeat(64)]);
  await assert.rejects(assertPreviewDatabaseIdentity(admin, previewSeedEnvironment), /RESOURCE_MISMATCH/);
  await admin.query("UPDATE deployment_environment_identity SET resource_fingerprint=$1", ["a".repeat(64)]);
  ...
  // M4 — évaluations, notes, coefficients, moyennes, verrouillage et isolation.
  assert.equal((await request(`/api/teaching-assignments/${assignment.id}/coefficient`, { method: "PUT", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ subjectCoefficient: 4 }) })).status, 200);
  assert.equal((await request(`/api/teaching-assignments/${classConflictAssignment.id}/coefficient`, { method: "PUT", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ subjectCoefficient: 3 }) })).status, 200);
  assert.equal((await request(`/api/teaching-assignments/${assignment.id}/coefficient`, { method: "PUT", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ subjectCoefficient: 9 }) })).status, 403);
  const assessmentTypes = await (await request("/api/assessment-types", { headers: { cookie } })).json();
  const devoirType = assessmentTypes.find((type) => type.code === "assignment");
  const compositionType = assessmentTypes.find((type) => type.code === "composition");
  assert.ok(devoirType && compositionType);
  assert.equal((await request("/api/assessment-types", { method: "POST", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ code: "teacher_global", name: "Type global enseignant" }) })).status, 403);
  const schoolBTypeResponse = await request("/api/assessment-types", { method: "POST", headers: { cookie: m3SchoolBCookie, "content-type": "application/json", "user-agent": "M3 School B" }, body: JSON.stringify({ code: "school_b_only", name: "Type établissement B" }) });
  assert.equal(schoolBTypeResponse.status, 201);
  assert.equal((await (await request("/api/assessment-types", { headers: { cookie } })).json()).some((type) => type.code === "school_b_only"), false);
  const createAssessment = async (teachingAssignmentId, assessmentTypeId, title, assessmentDate, maximumScore, coefficient, sessionCookie = cookie, userAgent) => {
    const response = await request("/api/assessments", { method: "POST", headers: { cookie: sessionCookie, "content-type": "application/json", ...(userAgent ? { "user-agent": userAgent } : {}) }, body: JSON.stringify({ academicYearId: currentYear.id, academicPeriodId: academicPeriodData.id, teachingAssignmentId, assessmentTypeId, title, assessmentDate, maximumScore, coefficient }) });
    return { response, data: await response.json() };
  };
  await admin.query(`CREATE OR REPLACE FUNCTION reject_m4_creation_audit() RETURNS trigger LANGUAGE plpgsql AS $$
    BEGIN IF NEW.action IN ('assessment_type.created','assessment.created') THEN RAISE EXCEPTION USING ERRCODE='XX000', MESSAGE='forced audit failure'; END IF; RETURN NEW; END $$`);
  await admin.query("CREATE TRIGGER reject_m4_creation_audit BEFORE INSERT ON audit_logs FOR EACH ROW EXECUTE FUNCTION reject_m4_creation_audit()");
  assert.equal((await request("/api/assessment-types", { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ code: "audit_atomic", name: "Audit atomique" }) })).status, 500);
  assert.equal(Number((await admin.query("SELECT count(*) total FROM assessment_types WHERE school_id=$1 AND code='audit_atomic'", [schoolA])).rows[0].total), 0);
  const failedAtomicAssessment = await createAssessment(assignment.id, devoirType.id, "Évaluation audit atomique", "2026-10-12", 20, 1);
  assert.equal(failedAtomicAssessment.response.status, 500);
  assert.equal(Number((await admin.query("SELECT count(*) total FROM assessments WHERE school_id=$1 AND title='Évaluation audit atomique'", [schoolA])).rows[0].total), 0);
  await admin.query("DROP TRIGGER reject_m4_creation_audit ON audit_logs");
  await admin.query("DROP FUNCTION reject_m4_creation_audit()");
  assert.equal((await request("/api/assessment-types", { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ code: "audit_atomic", name: "Audit atomique" }) })).status, 201);
  assert.equal((await createAssessment(assignment.id, devoirType.id, "Évaluation audit atomique", "2026-10-12", 20, 1)).response.status, 201);
  assert.equal(Number((await admin.query("SELECT count(*) total FROM assessments WHERE school_id=$1 AND title='Évaluation audit atomique'", [schoolA])).rows[0].total), 1);
  const mathEvaluationOne = await createAssessment(assignment.id, devoirType.id, "Devoir 1 de mathématiques", "2026-10-05", 20, 1, m2TeacherCookie);
  const mathEvaluationTwo = await createAssessment(assignment.id, compositionType.id, "Composition de mathématiques", "2026-10-10", 10, 2, m2TeacherCookie);
  const frenchEvaluation = await createAssessment(classConflictAssignment.id, devoirType.id, "Devoir de français", "2026-10-15", 20, 1, m3OtherTeacherCookie, "M3 Other Teacher");
  assert.deepEqual([mathEvaluationOne.response.status, mathEvaluationTwo.response.status, frenchEvaluation.response.status], [201, 201, 201]);
  assert.equal((await createAssessment(classConflictAssignment.id, devoirType.id, "Intrusion enseignant", "2026-10-15", 20, 1, m2TeacherCookie)).response.status, 404);
  assert.equal((await createAssessment(assignment.id, devoirType.id, "Accès comptable", "2026-10-15", 20, 1, m3AccountantCookie, "M3 Accountant")).response.status, 403);
  assert.equal((await createAssessment(assignment.id, devoirType.id, "Date hors période", "2026-11-15", 20, 1)).response.status, 400);
  assert.equal((await createAssessment(assignment.id, devoirType.id, "Barème invalide", "2026-10-15", 0, 1)).response.status, 400);
  assert.equal((await createAssessment(assignment.id, devoirType.id, "Coefficient invalide", "2026-10-15", 20, 0)).response.status, 400);
  const inactiveGradeAssignment = (await admin.query("INSERT INTO teaching_assignments(school_id,academic_year_id,teacher_id,class_id,subject_id,status) VALUES($1,$2,$3,$4,$5,'inactive') RETURNING id", [schoolA, currentYear.id, teacherA.id, currentClass.id, science.id])).rows[0];
  assert.equal((await createAssessment(inactiveGradeAssignment.id, devoirType.id, "Affectation inactive", "2026-10-15", 20, 1)).response.status, 404);
  const mathRoster = await request(`/api/assessments/${mathEvaluationOne.data.id}/roster`, { headers: { cookie: m2TeacherCookie } });
  assert.equal(mathRoster.status, 200);
  assert.equal((await mathRoster.json()).students.length, 2);
  const saveGrades = async (assessment, grades, sessionCookie, reason, userAgent) => request(`/api/assessments/${assessment.id}/grades`, { method: "POST", headers: { cookie: sessionCookie, "content-type": "application/json", ...(userAgent ? { "user-agent": userAgent } : {}) }, body: JSON.stringify({ assessmentVersion: assessment.version, grades, ...(reason ? { reason } : {}) }) });
  const firstSave = await saveGrades(mathEvaluationOne.data, [{ studentId: schoolAStudent, status: "scored", score: 12 }, { studentId: secondStudent.id, status: "absent" }], m2TeacherCookie);
  assert.equal(firstSave.status, 200);
  const firstSavedGrades = (await firstSave.json()).grades;
  assert.equal(Number(firstSavedGrades.find((grade) => grade.student_id === schoolAStudent).normalized_score), 12);
  const secondSave = await saveGrades(mathEvaluationTwo.data, [{ studentId: schoolAStudent, status: "scored", score: 8 }, { studentId: secondStudent.id, status: "absent" }], m2TeacherCookie);
  assert.equal(secondSave.status, 200);
  let secondBoundaryGrade = (await secondSave.json()).grades.find((grade) => grade.student_id === secondStudent.id);
  let boundaryResponse = await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "scored", score: 0, expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie);
  assert.equal(boundaryResponse.status, 200);
  secondBoundaryGrade = (await boundaryResponse.json()).grades[0];
  boundaryResponse = await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "scored", score: 10, expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie);
  assert.equal(boundaryResponse.status, 200);
  secondBoundaryGrade = (await boundaryResponse.json()).grades[0];
  assert.equal((await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "absent", score: 5, expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie)).status, 400);
  assert.equal((await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "scored", score: -1, expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie)).status, 400);
  assert.equal((await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "scored", expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie)).status, 400);
  assert.equal((await saveGrades(mathEvaluationTwo.data, [{ studentId: secondStudent.id, status: "absent", expectedVersion: secondBoundaryGrade.version }], m2TeacherCookie)).status, 200);
  assert.equal((await saveGrades(frenchEvaluation.data, [{ studentId: schoolAStudent, status: "scored", score: 13 }, { studentId: secondStudent.id, status: "excused" }], m3OtherTeacherCookie, null, "M3 Other Teacher")).status, 200);
  assert.equal((await saveGrades(mathEvaluationOne.data, [{ studentId: schoolAStudent, status: "scored", score: 21, expectedVersion: firstSavedGrades.find((grade) => grade.student_id === schoolAStudent).version }], m2TeacherCookie)).status, 400);
  const publish = async (assessment, sessionCookie, userAgent) => request(`/api/assessments/${assessment.id}/publish`, { method: "POST", headers: { cookie: sessionCookie, "content-type": "application/json", ...(userAgent ? { "user-agent": userAgent } : {}) }, body: JSON.stringify({ expectedVersion: assessment.version }) });
  for (const [assessment, sessionCookie, userAgent] of [[mathEvaluationOne.data, m2TeacherCookie], [mathEvaluationTwo.data, m2TeacherCookie], [frenchEvaluation.data, m3OtherTeacherCookie, "M3 Other Teacher"]]) assert.equal((await publish(assessment, sessionCookie, userAgent)).status, 200);
  const assessmentList = await request(`/api/assessments?academicYearId=${currentYear.id}&academicPeriodId=${academicPeriodData.id}&classId=${currentClass.id}`, { headers: { cookie } });
  assert.equal(assessmentList.status, 200);
  assert.ok((await assessmentList.json()).every((assessment) => assessment.student_count === 2));
  const publishedRoster = await (await request(`/api/assessments/${mathEvaluationOne.data.id}/roster`, { headers: { cookie: m2TeacherCookie } })).json();
  const publishedGrade = publishedRoster.students.find((student) => student.student_id === schoolAStudent);
  const correction = await request(`/api/grades/${publishedGrade.grade_id}`, { method: "PUT", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ status: "scored", score: 12, expectedVersion: publishedGrade.version, reason: "Correction contrôlée après vérification" }) });
  assert.equal(correction.status, 200);
  assert.equal((await request(`/api/grades/${publishedGrade.grade_id}`, { method: "PUT", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ status: "scored", score: 11, expectedVersion: publishedGrade.version, reason: "Tentative concurrente volontaire" }) })).status, 409);
  const history = await request(`/api/grades/${publishedGrade.grade_id}/history`, { headers: { cookie: m2TeacherCookie } });
  assert.equal(history.status, 200);
  assert.ok((await history.json()).some((event) => event.reason === "Correction contrôlée après vérification"));
  const lockEvaluation = await request(`/api/assessments/${mathEvaluationOne.data.id}/lock`, { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: 2 }) });
  assert.equal(lockEvaluation.status, 200);
  assert.equal((await request(`/api/grades/${publishedGrade.grade_id}`, { method: "PUT", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ status: "scored", score: 10, expectedVersion: (await correction.json()).version, reason: "Modification interdite après verrouillage" }) })).status, 409);
  assert.equal((await request(`/api/assessments/${mathEvaluationOne.data.id}/reopen`, { method: "POST", headers: { cookie: m2TeacherCookie, "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: 3, reason: "Réouverture demandée par enseignant" }) })).status, 403);
  assert.equal((await request(`/api/assessments/${mathEvaluationOne.data.id}/reopen`, { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: 3, reason: "Réouverture autorisée par la direction" }) })).status, 200);
  const studentGradeReport = await request(`/api/grade-reports?scope=student&academicPeriodId=${academicPeriodData.id}&studentId=${schoolAStudent.id || schoolAStudent}`, { headers: { cookie } });
  assert.equal(studentGradeReport.status, 200);
  const studentGradeRows = (await studentGradeReport.json()).rows;
  ...
  const mixedScaleReport = await (await request(`/api/grade-reports?scope=student&academicPeriodId=${academicPeriodData.id}&studentId=${schoolAStudent}`, { headers: { cookie } })).json();
  assert.equal(mixedScaleReport.rows.find((row) => row.subject_name === "Mathématiques").subject_average, "70");
  assert.equal(mixedScaleReport.rows.find((row) => row.subject_name === "Français").subject_average, "65");
  assert.equal(mixedScaleReport.rows[0].general_average, "68");
  assert.ok(mixedScaleReport.rows.every((row) => row.scale_max === "100.00"));
  ...
  const teacherGradeData = await teacherGradeReport.json();
  assert.deepEqual(new Set(teacherGradeData.rows.map((row) => row.subject_name)), new Set(["Mathématiques"]));
  assert.ok(teacherGradeData.rows.every((row) => row.general_average === undefined && row.authorized_average !== undefined && row.average_scope === "authorized_subjects"));
  assert.equal(teacherGradeData.summary.classAverage, undefined);
  const teacherStudentGrades = await (await request(`/api/students/${schoolAStudent}/grades?academicPeriodId=${academicPeriodData.id}`, { headers: { cookie: m2TeacherCookie } })).json();
  assert.ok(teacherStudentGrades.rows.every((row) => row.general_average === undefined && row.authorized_average !== undefined));
  assert.deepEqual(new Set(teacherStudentGrades.assessments.map((assessment) => assessment.subject_name)), new Set(["Mathématiques"]));
  ...
  assert.equal(cancelledReport.rows[0].general_average, "14.67");
  const classAverageClass = (await admin.query("INSERT INTO classes(school_id,academic_year_id,name,level,capacity) VALUES($1,$2,'Classe moyenne exacte','Test',10) RETURNING id", [schoolA, currentYear.id])).rows[0];
  const classAverageStudents = (await admin.query("WITH inserted AS (INSERT INTO students(school_id,matricule,first_name,last_name) VALUES($1,'AVG-A','Élève','Moyenne A'),($1,'AVG-B','Élève','Moyenne B') RETURNING id,matricule) SELECT id FROM inserted ORDER BY matricule", [schoolA])).rows;
  await admin.query("INSERT INTO enrollments(school_id,student_id,class_id,academic_year_id,status,enrolled_at) VALUES($1,$2,$4,$5,'active','2026-09-01'),($1,$3,$4,$5,'active','2026-09-01')", [schoolA, classAverageStudents[0].id, classAverageStudents[1].id, classAverageClass.id, currentYear.id]);
  const classAverageSubjects = (await admin.query("WITH inserted AS (INSERT INTO subjects(school_id,name,code) VALUES($1,'Matière moyenne 1','AVG-1'),($1,'Matière moyenne 2','AVG-2') RETURNING id,code) SELECT id FROM inserted ORDER BY code", [schoolA])).rows;
  const classAverageAssignments = [];
  for (const subject of classAverageSubjects) {
    const classAverageAssignment = (await admin.query("INSERT INTO teaching_assignments(school_id,academic_year_id,teacher_id,class_id,subject_id,status,subject_coefficient) VALUES($1,$2,$3,$4,$5,'active',1) RETURNING id", [schoolA, currentYear.id, teacherA.id, classAverageClass.id, subject.id])).rows[0];
    assert.equal((await request(`/api/teaching-assignments/${classAverageAssignment.id}/coefficient`, { method: "PUT", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ subjectCoefficient: 1 }) })).status, 200);
    classAverageAssignments.push(classAverageAssignment);
  }
  const averageAssessmentOne = await createAssessment(classAverageAssignments[0].id, devoirType.id, "Moyenne exacte matière 1", "2026-10-18", 20, 1);
  const averageAssessmentTwo = await createAssessment(classAverageAssignments[1].id, devoirType.id, "Moyenne exacte matière 2", "2026-10-19", 20, 1);
  assert.equal((await saveGrades(averageAssessmentOne.data, [{ studentId: classAverageStudents[0].id, status: "scored", score: 20 }, { studentId: classAverageStudents[1].id, status: "scored", score: 10 }], cookie)).status, 200);
  assert.equal((await saveGrades(averageAssessmentTwo.data, [{ studentId: classAverageStudents[0].id, status: "scored", score: 20 }], cookie)).status, 200);
  const averagePublishOne = await publish(averageAssessmentOne.data, cookie);
  assert.equal(averagePublishOne.status, 200, await averagePublishOne.text());
  const averagePublishTwo = await request(`/api/assessments/${averageAssessmentTwo.data.id}/publish`, { method: "POST", headers: { cookie, "content-type": "application/json" }, body: JSON.stringify({ expectedVersion: averageAssessmentTwo.data.version, confirmPending: true, reason: "Publication pour moyenne de classe exacte" }) });
  assert.equal(averagePublishTwo.status, 200, await averagePublishTwo.text());
  ...
  const schoolBSubject = (await admin.query("INSERT INTO subjects(school_id,name,code) VALUES($1,'Mathématiques B','MATH-B') RETURNING id", [schoolB])).rows[0];
  const schoolBAssignment = (await admin.query("INSERT INTO teaching_assignments(school_id,academic_year_id,teacher_id,class_id,subject_id) VALUES($1,$2,$3,$4,$5) RETURNING id", [schoolB, schoolBYear.id, teacherB.id, schoolBClass.id, schoolBSubject.id])).rows[0];
  const schoolBPeriod = (await admin.query("INSERT INTO academic_periods(school_id,academic_year_id,name,kind,position,starts_on,ends_on) VALUES($1,$2,'Trimestre B','trimester',1,'2026-09-01','2026-12-31') RETURNING id", [schoolB, schoolBYear.id])).rows[0];
  const schoolBType = (await admin.query("INSERT INTO assessment_types(school_id,code,name) VALUES($1,'assignment','Devoir') RETURNING id", [schoolB])).rows[0];
  await assert.rejects(admin.query("INSERT INTO assessments(school_id,academic_year_id,academic_period_id,teaching_assignment_id,assessment_type_id,title,assessment_date,maximum_score,created_by) VALUES($1,$2,$3,$4,$5,'Affectation étrangère','2026-10-05',20,$6)", [schoolA, currentYear.id, academicPeriodData.id, schoolBAssignment.id, devoirType.id, teacherA.id]), (error) => ["23503", "P0001"].includes(error.code));
  ...
  assert.deepEqual(ownStudentRows.map((student) => student.matricule).sort(), ["A-001", "AVG-A", "AVG-B", secondStudent.matricule].sort());
